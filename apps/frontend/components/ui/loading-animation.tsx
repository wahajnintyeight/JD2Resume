    ...
    return (
      <div className="flex flex-col items-center justify-center gap-6 p-8">
        <div className="relative">
          ...
          <div
            className="absolute inset-0 bg-gradient-to-r from-cyan-500 via-blue-500 to-fuchsia-500 blur-2xl opacity-40 animate-spin"
            style={{ animationDuration: '3s' }}
          />

          {/* Outer rotating border */}
          <div
            className={`relative ${sizeClasses[size]} border-2 border-cyan-400/30 bg-slate-950/50 backdrop-blur-sm flex items-center justify-center animate-spin`}
        ...
        {message && (
          <p
            className={`${textSizeClasses[size]} font-bold text-cyan-300 font-mono tracking-wider uppercase animate-pulse`}
          >
            {message}
          </p>
        )}
  ...
  if (variant === 'pulse') {
    return (
      <div className="flex flex-col items-center justify-center gap-6 p-8">
        ...
        {message && (
          <p
            className={`${textSizeClasses[size]} font-bold text-cyan-300 font-mono tracking-wider uppercase mt-8`}
          >
            {message}
          </p>
        )}
  ...
  if (variant === 'dots') {
    return (
      <div className="flex flex-col items-center justify-center gap-6 p-8">
        ...
        {message && (
          <p
            className={`${textSizeClasses[size]} font-bold text-cyan-300 font-mono tracking-wider uppercase`}
          >
            {message}
          </p>
        )}
...
export function LoadingOverlay({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0a0a0f] backdrop-blur-sm animate-in fade-in duration-200">
      ...
      <div
        className="absolute bottom-1/4 left-1/4 w-80 h-80 bg-gradient-to-tr from-fuchsia-500/15 via-pink-500/15 to-cyan-500/15 blur-3xl animate-pulse"
