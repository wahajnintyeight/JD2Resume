    ...
    return (
      <div className="flex flex-col items-center justify-center gap-4 p-8">
        <div className="relative">
          ...
          {/* Center icon */}
          <div className={`relative ${sizeClasses[size]} rounded-2xl bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center shadow-2xl animate-spin`}>
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-4 p-8">
        <div className="relative flex items-center justify-center">
          {/* Pulsing rings */}
          <div className={`absolute ${sizeClasses[size]} rounded-full border-4 border-indigo-500 animate-ping opacity-75`} />
          <div className={`absolute ${sizeClasses[size]} rounded-full border-4 border-indigo-400 animate-ping opacity-50`} style={{ animationDelay: '0.4s' }} />
          <div className={`absolute ${sizeClasses[size]} rounded-full border-4 border-indigo-300 animate-ping opacity-25`} style={{ animationDelay: '0.8s' }} />
          
          {/* Center dot */}
          <div className={`${sizeClasses[size]} rounded-full bg-gradient-to-br from-indigo-600 to-purple-600 shadow-lg animate-pulse`} />
        </div>
        
        {message && (
          <p className={`${textSizeClasses[size]} font-medium text-slate-600 mt-8`}>
            {message}
          </p>
        )}
      </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-4 p-8">
        <div className="flex gap-2">
          ...
          <div className="w-3 h-3 rounded-full bg-gradient-to-br from-indigo-600 to-purple-600 animate-bounce" style={{ animationDelay: '0.15s' }} />
          <div className="w-3 h-3 rounded-full bg-gradient-to-br from-indigo-600 to-purple-600 animate-bounce" style={{ animationDelay: '0.3s' }} />
        </div>
        
        {message && (
          <p className={`${textSizeClasses[size]} font-medium text-slate-600`}>
            {message}
          </p>
        )}
      </div>
